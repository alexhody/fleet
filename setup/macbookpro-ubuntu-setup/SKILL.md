...
| Thermal/power | mbpfan, `thermald` masked (~18 % faster sustained builds), CPU undervolted 65 mV (skipped for one boot after a crash), battery stops charging at 80 % (set again on every boot), never sleeps, ignores lid | `/usr/local/sbin/{bclm,undervolt}`, `{battery-limit,undervolt}.service`, `/etc/modprobe.d/msr-writes.conf`, `/etc/systemd/logind.conf.d/99-no-sleep.conf` |
